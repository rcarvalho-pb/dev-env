...
-- 1. Gerenciamento de Pacotes via vim.pack
vim.pack.add({
  "Saghen/blink.cmp",
  "williamboman/mason.nvim",
  "williamboman/mason-lspconfig.nvim",
  "neovim/nvim-lspconfig",
  "creativenull/efmls-configs-nvim", -- Coleção de configs para linters/formatadores
})
...
-- 3. Autocmd LspAttach: Atalhos e Inlay Hints Nativos
vim.api.nvim_create_autocmd("LspAttach", {
  ...
  callback = function(event)
    ...
    -- Helper local para atalhos do buffer
    ...
    -------------------------------------------------------------------
    -- ATALHOS PADRÃO DO LSP E FORMATAÇÃO
    -------------------------------------------------------------------
    ...
    -- Atalho nativo para reformatar o código usando o efm/LSP
    map({ "n", "v" }, "<leader>f", function()
      vim.lsp.buf.format({ async = true })
    end, "Formatar Buffer/Seleção")
...
-- 4. Mason & LSPConfig + Integração EFM
require("mason").setup()
...
-- Módulos de formatadores/linters do efmls-configs
...
mason_lspconfig.setup({
  ensure_installed = {
    ...
    "bashls",
    "templ",
    "efm", -- Adicionado o servidor bridge do EFM
  },
  automatic_installation = true,

  handlers = {
    -- Handler Genérico para a maioria dos LSPs
    ...
    -- Customização do EFM Language Server
    ["efm"] = function()
      lspconfig.efm.setup({
        capabilities = capabilities,
        filetypes = { "lua", "javascript", "typescript", "html", "css", "sh" },
        init_options = { 
